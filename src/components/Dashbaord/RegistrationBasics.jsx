...
const RegistrationBasics = () => {
  ...
  const [fullName, setFullName] = useState(basics.name || "");
  const [email, setEmail] = useState(basics.email || "");
  const [phoneValue, setPhoneValue] = useState(basics.phone || "");
  const [phoneCountryCode, setPhoneCountryCode] = useState(basics.countryCode || "");
  const [qidPassport, setQidPassport] = useState(basics.identificationNumber || "");

  // Initialize dates
  const initialDobRaw = basics.dateOfBirth || "";
  let initialDobValue = "";
  if (initialDobRaw) {
    const [year, month, day] = initialDobRaw.split("-");
    if (year && month && day) {
      initialDobValue = `${day}-${month}-${year}`;
    }
  }

  const [dobValue, setDobValue] = useState(initialDobValue); // display as DD-MM-YYYY
  const [dobRaw, setDobRaw] = useState(initialDobRaw); // raw YYYY-MM-DD for backend
  const [gender, setGender] = useState(basics.gender || "");
  const [nationalityValue, setNationalityValue] = useState(basics.nationality || "Qatar");
  const [residenceValue, setResidenceValue] = useState(basics.countryOfResidence || "Qatar");

  // Handle boolean or string for visaStatus
  const getVisaStatus = () => {
    if (basics.workVisa === true) return "yes";
    if (basics.workVisa === false) return "no";
    return "";
  };
  const [visaStatus, setVisaStatus] = useState(getVisaStatus());

  const [timeZone, setTimeZone] = useState(basics.timeZone || "");
  const [photoName, setPhotoName] = useState(basics.photoFile?.name || "");
  const [photoFile, setPhotoFile] = useState(basics.photoFile || null);
  const [srcImage, setSrcImage] = useState(null);
  const [crop, setCrop] = useState({ aspect: 1 });
  const [croppedImageUrl, setCroppedImageUrl] = useState(basics.photoUrl || "");
  const [showCropper, setShowCropper] = useState(false);
  const [pendingPhotoFile, setPendingPhotoFile] = useState(null);
  const [pendingPhotoName, setPendingPhotoName] = useState("");
  const [about, setAbout] = useState(basics.about || "");
  ...
  const handleClear = () => {
    ...
    setDobRaw("");
    setGender("");
    setNationalityValue("Qatar");
    setResidenceValue("Qatar");
    setVisaStatus("");
    setAbout("");
    setPhotoName("");
    setPhotoFile(null);
              ...
              <div className="registration-row">
                <div className="registration-field">
                  <div className="registration-flag-input">
                    <span className="registration-flag-icon">
                    ...
                    <span style={{ color: '#666', marginRight: '4px' }}>+974</span>
                    <input
                      type="tel"
                      style={{ border: 'none', background: 'transparent', color: 'inherit', width: '100%', outline: 'none' }}
                      placeholder="Phone number*"
                      value={phoneValue.startsWith('974') ? phoneValue.slice(3) : phoneValue}
                      onChange={(e) => {
                        const onlyDigits = e.target.value.replace(/[^0-9]/g, "");
                        setPhoneValue('974' + onlyDigits);
